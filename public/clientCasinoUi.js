...
function renderCasinoModal() {
  ...
  if (spinId && card.dataset.casinoSpinId === spinId && card.querySelector("[data-casino-reel]")) return;
  const existingStake = card.querySelector("[name=casino-desk-stake]");
  if (existingStake) {
    casinoDraft = {
      stake: existingStake.value,
...
function onCasinoSubmit(event) {
  ...
  const color = form.querySelector("input[name=casino-desk-color]:checked")?.value || "red";
  const stake = Math.max(1, Math.floor(Number(form.querySelector("[name=casino-desk-stake]")?.value) || 0));
  const requestId = host.createRequestId("casino");
  emitWithTimeout(host.emitServer, "place-casino-bet", { color, stake, requestId }, response => {
