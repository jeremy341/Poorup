...
export function createMusicPlayer(options = {}) {
  ...
  try { const saved = JSON.parse(storage.getItem(PREFS) || "{}"); volume = Number.isFinite(Number(saved.volume)) ? clamp(saved.volume, 0, 1) : 0.16; shuffle = saved.shuffle === true; loop = saved.loop !== false; if (saved.theme) theme = sanitizeThemeId(saved.theme, manifest); } catch {}
  ...
  const rebuildQueue = () => { const ids = (manifest.themes[theme] || [resolveThemeTrack(theme, manifest)]).filter(id => sanitizeTrackId(id, manifest)); queue = [...new Set(ids.length ? ids : ["pondering-the-cosmos"])]; if (shuffle && queue.length > 1) queue = queue.slice(1).concat(queue[0]); queueIndex = Math.max(0, queue.indexOf(current)); history = []; };
  ...
  function load(id, duration = 350) {
    ...
    const token = ++transition; const incoming = 1 - active; const element = audios[incoming]; if (!element) return false; const prior = current;
    ...
    const play = () => { if (token !== transition) return; try { const result = element.play?.(); if (result?.then) result.then(finish).catch(() => announceStatus("autoplay-blocked")); else finish(); } catch { announceStatus("autoplay-blocked"); } };
    if (typeof element.addEventListener === "function") {
      const onReady = () => { element.removeEventListener?.("canplay", onReady); play(); };
      const onError = () => { if (token === transition) { current = prior; announceStatus("track-error"); } };
    ...
    if (reducedMotion || duration <= 0) { /* media promise controls completion */ }
    persist(); return true;
  }
  ...
  function setTheme(id, { userInitiated = false } = {}) { const nextTheme = sanitizeThemeId(id, manifest); if (!userInitiated && nextTheme === theme && mode !== "CUSTOM") return; theme = nextTheme; mode = "AUTO THEME"; loop = true; current = resolveThemeTrack(theme, manifest); rebuildQueue(); return load(current, 650); }
