...
export function createMusicPlayer(options = {}) {
  ...
  function load(id, duration = 350, restore = { current, queueIndex }) {
    ...
    if (typeof element.addEventListener === "function") {
      ...
      const onError = () => { if (token === transition) { current = prior; queueIndex = priorIndex; element.pause?.(); setVolumes(); persist(); announceStatus("track-error"); } };
  ...
  function selectTrack(id) { if (!sanitizeTrackId(id, manifest)) return false; const restore = { current, queueIndex }; mode = "CUSTOM"; if (!queue.includes(id)) queue.push(id); queueIndex = queue.indexOf(id); history.push(current); return load(id, 350, restore); }
  function setTheme(id, { userInitiated = false } = {}) { const restore = { current, queueIndex }; const nextTheme = sanitizeThemeId(id, manifest); if (!userInitiated && nextTheme === theme && mode !== "CUSTOM") return; theme = nextTheme; mode = "AUTO THEME"; loop = true; const target = resolveThemeTrack(theme, manifest); rebuildQueue(); if (!target) { current = restore.current; queueIndex = restore.queueIndex; announceStatus("track-unavailable"); persist(); return false; } current = target; return load(target, 650, restore); }
