...
export function initAdminAiProvider() {
  ...
  async function save(event) {
    ...
    render();
    try {
      const response = await fetch(editing ? `${API_ROOT}/providers/${encodeURIComponent(model.selectedId)}` : `${API_ROOT}/providers`, { method: editing ? 'PATCH' : 'POST', headers: sessionHeaders(true), credentials: 'include', body: JSON.stringify(body), cache: 'no-store' });
      const payload = await response.json().catch(() => ({}));
  ...
  async function testProfile(id) {
    ...
    render();
    try {
      const response = await fetch(`${API_ROOT}/providers/${encodeURIComponent(id)}/test`, { method: 'POST', headers: sessionHeaders(true), credentials: 'include', body: '{}', cache: 'no-store' });
      const payload = await response.json().catch(() => ({}));
  ...
  async function activateProfile(id) {
    ...
    render();
    try {
      const response = await fetch(`${API_ROOT}/providers/${encodeURIComponent(id)}/activate`, { method: 'POST', headers: sessionHeaders(true), credentials: 'include', body: '{}', cache: 'no-store' });
      const payload = await response.json().catch(() => ({}));
