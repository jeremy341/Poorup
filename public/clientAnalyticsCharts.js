const CHART_COLORS = Object.freeze({ line: 'var(--gold-300)', bar: 'var(--green-status)', muted: 'var(--line-default)' });
let chartSequence = 0;
...
export function renderAnalyticsChart(container, series, options = {}) {
  ...
  const captionId = labelledBy && /^[A-Za-z][A-Za-z0-9_-]*$/.test(labelledBy) ? ` id="${labelledBy}"` : '';
