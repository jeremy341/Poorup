...
// Contract 7b: requested room codes must never Map-overwrite either. A
// colliding request mints a fresh code; the original room survives.
async function testRequestedRoomCodeCollision() {
  const manager = new RoomManager();
  const seeded = manager.createRoom({ socketId: 'socket-seed', clientId: 'client-seed', nickname: 'Seed', roomCode: 'BBBBBB' });
  const second = manager.createRoom({ socketId: 'socket-two', clientId: 'client-two', nickname: 'Two', roomCode: 'BBBBBB' });
  assert.notEqual(second.roomCode, 'BBBBBB');
  assert.equal(manager.rooms.size, 2);
  assert.equal(manager.rooms.get('BBBBBB'), seeded);
}
...
const GOLDEN_GARBAGE = {"matchId":"xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx","completedAt":"<iso>","durationSeconds":0,"roundCount":0,"roomVisibility":"public","participants":[],"globalEvents":["a","b"],"eventCombinations":[],"tradesCompleted":0,"auctionsCompleted":0,"casino":[{"accountId":null,"bets":2.5,"net":0}],"market":[{"accountId":"m1","positions":{}}],"playerContracts":[{"id":"i","kind":"loan","fromAccountId":null,"toAccountId":null,"fromPlayerId":null,"toPlayerId":null,"amount":0,"status":"ssssssssssssssssssss","premiumRate":0,"equityShare":0,"collateralTileIndex":null,"propertyIndex":null,"conversionShare":0,"equityControl":null}]};
...
// The settlement/idempotency and collateral rules intentionally changed this
// fixture: one account row is recorded per match and bank collateral defaults
// are surfaced as a dedicated achievement.
GOLDEN_ACH.splice(GOLDEN_ACH.findIndex(entry => entry.accountId === 'a2' && entry.achievementId === 'debt-free'), 1);
GOLDEN_ACH.splice(GOLDEN_ACH.findIndex(entry => entry.accountId === 'a1' && entry.achievementId === 'crisis-manager'), 0, { accountId: 'a1', achievementId: 'collateral-damage', title: 'COLLATERAL DAMAGE', rarity: 'RARE', body: 'A loan default cost the collateral deed.' });
for (let index = GOLDEN_ACH.length - 1; index >= 0; index -= 1) {
  if ((GOLDEN_ACH[index].accountId === 'a1' && GOLDEN_ACH[index].achievementId === 'crisis-manager')
    || GOLDEN_ACH[index].achievementId === 'double-headline') GOLDEN_ACH.splice(index, 1);
}
...
const GOLDEN_ACCT = {"id":"<acct>","username":"goldone","displayName":"G","color":"#d74438","avatarGrid":null,"stats":{"gamesPlayed":2,"wins":1,"bankruptcies":0,"auctionWins":2,"rentCollected":300,"eventSurvival":1,"casinoNet":-20,"marketProfit":10,"playerLoansGiven":4,"playerLoansRepaid":0,"playerLoanDefaults":2,"equityDeals":2,"bankLoansTaken":2,"bankLoanRepayments":1,"bankLoanDefaults":1,"patrolBest":0,"patrolAceRuns":0},"history":[{"matchId":"game-1","playedAt":"<iso>","result":"ROUND","won":false,"endingCash":2.9,"properties":0},{"matchId":"game-1","playedAt":"<iso>","result":"WIN","won":true,"endingCash":1234,"properties":3}],"achievements":[],"matchHistory":[{"matchId":"game-1"}],"recordedMatchIds":["game-1"],"privacy":{"history":"friends","achievements":"friends","friendRequests":"everyone","roomInvites":"friends"},"recentClearedAt":null,"createdAt":"<iso>"};
GOLDEN_ACCT.stats.gamesPlayed = 1;
GOLDEN_ACCT.stats.casinoNet = -10;
GOLDEN_ACCT.stats.marketProfit = 5;
GOLDEN_ACCT.stats.playerLoansGiven = 2;
GOLDEN_ACCT.stats.playerLoanDefaults = 1;
GOLDEN_ACCT.stats.equityDeals = 1;
GOLDEN_ACCT.stats.bankLoanDefaults = 0;
GOLDEN_ACCT.history = [{ matchId: 'game-1', playedAt: '<iso>', result: 'WIN', won: true, endingCash: 1234, properties: 3 }];
