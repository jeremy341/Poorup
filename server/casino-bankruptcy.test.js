...
check('setRoomSetting — string settings use their domain normalizers', () => {
  const room = lobby();
  room.setRoomSetting('bankruptMode', '  elim  ');
  assert.equal(room.settings.bankruptMode, 'elim');
  const legacyBankruptMode = room.setRoomSetting('bankruptMode', 5);
  assert.equal(legacyBankruptMode.rejected, false);
  assert.equal(legacyBankruptMode.value, 'elim');
  assert.equal(room.settings.bankruptMode, 'elim');
...
// ---------------------------------------------------------------------------
// Legacy snapshots may still say `debt`, but a bankruptcy declaration always
// eliminates the seat; debt only allows rescue actions before that declaration.
// ---------------------------------------------------------------------------

check('legacy debt-mode snapshot still eliminates on bankruptcy', () => {
  const room = makeStartedRoom(false);
...
check('legacy debt-mode bankruptcy clears the debtor gate and liquidates market positions', () => {
  const room = makeStartedRoom(false);
  const game = room.game;
  game.settings.bankruptMode = 'debt';
  const player = game.getPlayerBySocket('socket-a');
